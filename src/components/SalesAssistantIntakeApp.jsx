...
import { SALES_ASSISTANT_INTAKE_STEPS, SALES_ASSISTANT_QUESTION_STEPS } from '../intake/salesAssistantIntakeConfig';
...
function renderRichParts(parts) {
  return parts.map((part, index) => {
    if (part.type === 'link') {
      return (
        <Link key={`${part.value}-${index}`} to={part.to} className="inline-legal-link">
          {part.value}
        </Link>
      );
    }
...
function renderConsentField(field, formData, updateFormData) {
  ...
  return (
    <div key={field.name} className="field-group field-group-consent">
...
function buildSubmissionPayload(submissionId, formData) {
  ...
  const answersOrdered = SALES_ASSISTANT_QUESTION_STEPS.map((step, index) => {
  ...
  const answers = Object.fromEntries(answersOrdered.map((entry) => [entry.key, entry.answer]));
  ...
  return {
    ...
    consent: {
      legalAccepted: Boolean(formData.legalAccepted),
      legalAcceptedAt: formData.legalAccepted ? submittedAt : '',
    ...
    rawIntake: {
      ...
      landingVariant: 'sales_assistant_legacy',
      questionCount: SALES_ASSISTANT_QUESTION_STEPS.length,
      submittedAt,
      fullName: formData.fullName || '',
      businessName: formData.businessName || '',
      contactEmail: formData.contactEmail || '',
    ...
    <div className="root-wrapper">
      ...
      >
        ...
        <p className="success-fine">
          Sandler Agent Intake • Workflow, systems, and trust-boundary review
        </p>
      </motion.div>
    </div>
  );
...
export default function SalesAssistantIntakeApp() {
  useSecretRouteMeta('Qortana Sales Assistant Quiz');
  ...
  const currentStep = SALES_ASSISTANT_INTAKE_STEPS[currentStepIdx];
  const progress = ((currentStepIdx + 1) / SALES_ASSISTANT_INTAKE_STEPS.length) * 100;
  const isLastStep = currentStepIdx === SALES_ASSISTANT_INTAKE_STEPS.length - 1;
        ...
        <div className="card-header">
          ...
          <span className="step-counter">
            ...
            <span className="step-total">
              {String(SALES_ASSISTANT_INTAKE_STEPS.length).padStart(2, '0')}
            </span>
          </span>
        </div>
              ...
              <div className="fields-container">
                {currentStep.fields.map((field) => {
                  if (field.type === 'consent') {
                    return renderConsentField(field, formData, updateFormData);
                  }
